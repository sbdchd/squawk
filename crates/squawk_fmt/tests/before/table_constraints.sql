...
create table named_constraints (
  ...
  CONSTRAINT long_check CHECK (a_very_long_check_left_hand_expression_name > a_very_long_check_right_hand_expression_name),
...
create table commented_inherited_not_null_constraint (
  inherited_column bigint,
  /* constraint */ CONSTRAINT /* name */ inherited_not_null /* not */ NOT /* null */ NULL /* column */ inherited_column /* no */ NO /* inherit */ INHERIT
);
...
create table temporal_constraints (
  ...
  FOREIGN KEY (parent_id, PERIOD valid_at) REFERENCES temporal_parents (id, PERIOD valid_at)
);
...
create table commented_temporal_constraints (
  ...
  parent_id bigint,
  valid_at daterange,
  PRIMARY /* before primary key */ KEY /* before primary opening paren */ (id /* before primary comma */, /* before valid column */ valid_at /* before without */ WITHOUT /* before overlaps */ OVERLAPS /* before primary closing paren */),
  FOREIGN /* before foreign key */ KEY /* before from opening paren */ (parent_id /* before period comma */, /* before from period */ PERIOD /* before from period column */ valid_at /* before from closing paren */) /* before references */ REFERENCES /* before referenced table */ temporal_parents /* before to opening paren */ (id /* before referenced period comma */, /* before referenced period */ PERIOD /* before referenced period column */ valid_at /* before to closing paren */)
);
...
create table source_ordered_referential_actions (
  id bigint,
  parent_id bigint REFERENCES parents /* before update on */ ON /* before update */ UPDATE /* before update action */ CASCADE /* before delete on */ ON /* before delete */ DELETE /* before delete action */ RESTRICT,
  FOREIGN KEY (parent_id) REFERENCES parents (id) /* before update on */ ON /* before update */ UPDATE /* before update action */ SET /* before update null */ NULL /* before delete on */ ON /* before delete */ DELETE /* before delete action */ NO /* before action */ ACTION
);
