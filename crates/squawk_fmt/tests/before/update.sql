...
update foo set a = (select count(*) from a_very_long_relation_name_that_forces_the_subquery_to_wrap_ab);

update foo set a = array[a_very_long_first_array_expression, a_very_long_second_array_expression, a_very_long_third_array_expression];

update foo set a = coalesce(a_very_long_first_expression, a_very_long_second_expression, a_very_long_third_expression);

UPDATE foo SET a = 1 RETURNING (aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa, bbbbbbbbbbbbbbbbbbbbbbbbbbbb);

UPDATE foo SET a = 1 RETURNING aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa, bbbbbbbbbbbbbbbbbbbbbbbbbbbb;

update foo set -- before column
a = f(1);

update foo set -- before column
a = 1, b = 2;

update foo set a = 1 returning -- before target
f(a);
