    ...
    (
     ...
     multi_byte_chars: $multi_byte_chars:expr,
        ...
        fn $test_name() {
            ...
            let expected_lines: Vec<TextSize> = $lines
                .into_iter()
                .map(<TextSize as From<u32>>::from)
                .collect();
...
fn test_every_chars() {
    ...
    let mut col_utf16 = 0;
    let mut col_utf32 = 0;
    // SQUAWK: the shuffled text contains `\r`s, which break a line for us but
    // not for upstream, so we have to peek to tell a lone `\r` from a `\r\n`.
