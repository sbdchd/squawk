...
pub(crate) fn resolve_name_ref(
    ...
    match context {
        ...
        NameRefClass::Policy => {
            let (policy_name, on_table_path) = name_ref.syntax().ancestors().find_map(|a| {
                if let Some(policy) = ast::DropPolicy::cast(a.clone()) {
                    Some((
                        policy.name_ref(),
                        policy.on_table().and_then(|on_table| on_table.path()),
                    ))
                    ...
                    ast::ObjectPolicy::cast(a).map(|policy| (policy.name_ref(), policy.path()))
                }
            })?;
        ...
        NameRefClass::Rule => {
            let (rule_name, on_table_path) = name_ref.syntax().ancestors().find_map(|a| {
                if let Some(drop_rule) = ast::DropRule::cast(a.clone()) {
                    ...
                    Some((object_rule.name_ref(), object_rule.path()))
                } else {
                    ast::AlterTable::cast(a).map(|alter_table| {
                        (
                            Some(name_ref.clone()),
        ...
        NameRefClass::Conversion => {
            let (schema, conversion_name) = name::schema_and_name(name_ref);
            let position = name_ref.syntax().text_range().start();
            let binder = bind(db, file);
            let schemas = binder.resolved_schemas(position, schema.as_ref());
        ...
        NameRefClass::OperatorClass => {
            let (schema, operator_class_name) = name::schema_and_name(name_ref);
            let position = name_ref.syntax().text_range().start();
            let binder = bind(db, file);
            let schemas = binder.resolved_schemas(position, schema.as_ref());
            let ptr =
        ...
        NameRefClass::TextSearchConfiguration => {
            let (schema, configuration_name) = name::schema_and_name(name_ref);
            let position = name_ref.syntax().text_range().start();
            let binder = bind(db, file);
            let schemas = binder.resolved_schemas(position, schema.as_ref());
        ...
        NameRefClass::TextSearchParser => {
            let (schema, parser_name) = name::schema_and_name(name_ref);
            let position = name_ref.syntax().text_range().start();
            let binder = bind(db, file);
            let schemas = binder.resolved_schemas(position, schema.as_ref());
        ...
        NameRefClass::TextSearchTemplate => {
            let (schema, template_name) = name::schema_and_name(name_ref);
            let position = name_ref.syntax().text_range().start();
            let binder = bind(db, file);
            let schemas = binder.resolved_schemas(position, schema.as_ref());
            let ptr =
...
fn resolve_alter_column_relation_path(name_ref: &ast::NameRef) -> Option<ast::Path> {
    ...
    {
        return alter_materialized_view.path();
    }
    None
}
...
fn find_column_in_create_table_impl(
    ...
    if skip_column_count == 0
        && let Some(parent_path) = create_table.partition_of().and_then(|p| p.path())
    {
